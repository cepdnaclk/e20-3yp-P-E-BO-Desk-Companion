...
const DashboardScreen = () => {
  ...
  const [showSetupIncompleteDetails, setShowSetupIncompleteDetails] =
    useState(false);
...
useEffect(() => {
  ...
  // Only check completeness after initial data loaded
  if (!(wifiLoaded && devsLoaded && profileLoaded)) {
    setShowSetupWarning(true);
    return;
  }
  // On any relevant data update, re-check completeness
  const hasWifi = wifiDetails.wifiSSID && wifiDetails.wifiSSID.trim();
  const hasPebos = userPebos.length > 0;
  const hasProfileImage = userProfileImage && userProfileImage.trim();
  const hasUserName =
    userName && userName.trim() && userName !== "Guest" && userName !== "User";
  setShowSetupWarning(!(hasWifi && hasPebos && hasProfileImage && hasUserName));
}, [
  wifiLoaded,
  devsLoaded,
  profileLoaded,
  currentUser,
  isLoading,
  wifiDetails, // Changes in WiFi config
  userPebos, // Changes in device list
  userProfileImage, // Profile image update
