...
def detect_double_tap(touch_pin, max_interval=0.5, min_interval=0.1):
    """
...
async def play_music(song="Perfect Ed Sheeran", controller=None, touch_pin=17):
    """
    ...
    touch_thread = threading.Thread(target=detect_double_tap, args=(touch_pin, 0.5, 0.1))
    touch_thread.start()
    ...
    try:
        ...
        while ffplay_process.poll() is None:  # While ffplay is running
            if stop_music_flag:
                ...
                print("Music stopped, any thing else.")
                break
