#!/usr/bin/env python3
"""
PEBO Assistant (merged, fixed)

- Strict [emotion,answer] parsing with retry; use explicit user sentiment for animation
- Sanitized, concise TTS with lock; no emotion labels spoken; stage directions removed
- Robust Edge TTS with gain boost and offline espeak fallback; audio-guarded
- Eyes/arms unified with proper thread stop/join and eyes.Default capitalization
- Keeps: reminders, tasks (Firebase), QR, inter-device audio, math quick/continue, songs
"""
...
ROLE_PROMPT = (
    "Act as 'pebo'—empathetic, concise, a bit cute; avoid stage directions; "
    "never say emotion labels; keep replies ~25 tokens and supportive."
)
...
def _redact_emotion_mentions(text: str) -> str:
    ...
    return cleaned if cleaned else "Here for anything needed."
...
def classify_intent(text: str) -> str:
    ...
    if re.search(r"reminder|task|todo", t):
        return "task"
    if re.search(r"what\s+is\s+.*?(\+|\-|x|×|/)|\d+\s*(\+|\-|x|×|/)\s*\d+", t):
        return "math"
...
def react_detected_emotion_label(label: str | None):
    ...
    if key == "LOVE":
        return love()
    if key == "TIRED":
        return sad()  # show tired eyes/slow arms; reuse sad eyes if no dedicated
    if key in {"CONFUSED", "FEAR", "STRESSED"}:
        return sad()
    return normal()
...
async def handle_qr_intent():
    ...
    await asyncio.gather(
        ...
        asyncio.to_thread(run_emotion, None, lambda stop_event: eyes.QR(device_id, stop_event=stop_event), 15)
    )
    await asyncio.to_thread(normal)
...
async def start_loop():
    ...
    while True:
        ...
        # LLM with strict schema; model emotion ignored for animation
        ...
        def parse_emotion_reply(reply: str):
            m = re.match(r'\[(Happy|Sad|Angry|Normal|Love),(.+?)\]', (reply or "").strip())
            if m:
        ...
        try:
            ...
            model_emotion, answer = parse_emotion_reply(reply)
            if not model_emotion:
                ...
                model_emotion, answer = parse_emotion_reply(reply)
            if not model_emotion:
                model_emotion, answer = "Normal", reply
        except gcore_exceptions.NotFound as e:
            print(f"Model not found: {e}. Check name or quota.")
            model_emotion, answer = "Normal", "Taking a tiny break."
        except Exception as e:
            print(f"Gemini error: {e}")
            model_emotion, answer = "Normal", "Hiccup—try again?"
