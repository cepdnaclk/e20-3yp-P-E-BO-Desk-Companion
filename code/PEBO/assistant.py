...
async def start_assistant_from_text(prompt_text, controller):
    ...
    emotion_method = emotion_methods.get(emotion, controller.normal)
...
async def monitor_for_trigger(user, emotion):
    ...
    controller = RobotController()  # Initialize robot controller
    controller.normal()  # Set normal state (Default eyes, Tired arms) until trigger
    ...
    while True:
        ...
        if text:
            if re.search(trigger_pattern, text, re.IGNORECASE):
                ...
                # Run hi expression and voice output simultaneously
                hi_task = asyncio.to_thread(controller.hi)  # Run hi in a thread
                voice_task = speak_text("Hello! I'm your pebo.", controller)  # Run voice output
                await asyncio.gather(hi_task, voice_task)  # Run both tasks simultaneously
                await start_assistant_from_text(f"I am {user}. I look {emotion}. Ask why.", controller)
...
if __name__ == "__main__":
    try:
        ...
        # Cleanup is handled by RobotController in arm_eyes.py
